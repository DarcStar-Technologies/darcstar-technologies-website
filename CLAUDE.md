...
- **End-user portal (`/account`, #96) activates the dormant `user` role** — a self-service surface (own profile via `auth.api.updateUser`/`changePassword`, own messages) **separate from `/admin`**, gated only by sign-in (no `isStaff`). Message ownership: `contact_submission.user_id` (nullable FK, `set null`), linked via `linkSubmissionsToUser` (`contact-ownership.ts`) on signed-in submit, admin-create (vouch), or self-registered email-verify (PR 2). Every `/account` query is scoped to `locals.user.id` (the load-bearing isolation). Delivered in **two PRs**: PR 1 = ownership + portal; **PR 2 = public sign-up + email verification + Turnstile** (re-opens #48 — must mark existing staff `emailVerified` before `requireEmailVerification`, and scope captcha to `/sign-up/email` or it breaks login). → [auth](docs/auth.md)
