...
- **Signed-in page views skip the DB** — `session.cookieCache` (`auth-options.ts`, 5-min `maxAge`) serves the site-wide `getSession` from a signed `session_data` cookie snapshot, not a per-view DB read; sign-out clears the snapshot. Behavioral, so it's NOT mirrored to the CLI config. → [auth](docs/auth.md)
