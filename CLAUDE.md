...
- **Signed-in page views skip the DB** — `session.cookieCache` (`auth-options.ts`, 5-min `maxAge`) serves the site-wide `getSession` from a signed `session_data` cookie snapshot, not a per-view DB read; sign-out clears the snapshot. Behavioral, NOT mirrored to the CLI config. **But** the hook resolves `/admin`+`/login`+`/api/auth/*` **authoritatively** (`disableCookieCache`), so roster force-logout/disable are immediate there — the cache only fronts the navbar (staleness is cosmetic). → [auth](docs/auth.md)
