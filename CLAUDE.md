...
- **Better Auth gates the `/admin` submissions view (#69)** — email/password sign-in exists, but **public sign-up stays disabled** (`disableSignUp`); the only way to make an operator is `pnpm admin:create`. Sign-in goes through the **client** (`auth-client.ts`) so it hits the router's **DB-backed rate limiter** (`rateLimit.storage: 'database'` → `rate_limit` table; a direct `auth.api` call would skip it). `handleBetterAuth` now populates `locals.user` for `/api/auth/*` + `/admin` + `/login` (de-localized match) but mounts the API only under `/api/auth/*`. Run `pnpm db:push` (adds `rate_limit`) + `pnpm admin:create` before the admin area works. → [auth](docs/auth.md)
