...
- **Dialog** — `src/lib/components/ContactDialog.svelte` (Skeleton `<Dialog>`, headless → styled with `glass-panel`), rendered **once** in `+layout.svelte`. Skeleton provides focus-trap, scroll-lock, Esc, and outside-click close; `z-[60]` clears the header (`z-50`) and scrim (`z-40`). Fields use the reusable **`glass-field`** utility (recessed/beveled/grainy wells carved into the panel — not Skeleton `.input`, whose Windows bugfix forces an opaque fill). The interest picker is a custom accessible dropdown — **`GlassSelect.svelte`**, the ARIA _select-only combobox_ pattern (a `role=combobox` button that owns the keyboard + `aria-activedescendant` over a `role=listbox` popup) — instead of a native `<select>`, so its popup wears the theme (glass surface, `primary`-tinted active option) and renders identically across browsers; its value rides a hidden input into the form's FormData. (`color-scheme: dark` on `<html>` still darkens the _other_ native controls — scrollbars, autofill.) Errors use `text-error-400`, success `text-success-400` (WCAG floors, see [styling](styling.md)).
