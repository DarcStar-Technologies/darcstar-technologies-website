...
- **Dialog** — `src/lib/components/ContactDialog.svelte` (Skeleton `<Dialog>`, headless → styled with `glass-panel`), rendered **once** in `+layout.svelte`. Skeleton provides focus-trap, scroll-lock, Esc, and outside-click close; `z-[60]` clears the header (`z-50`) and scrim (`z-40`). Fields are hand-styled translucent (not Skeleton `.input`, whose Windows bugfix forces an opaque fill); errors use `text-error-400`, success `text-success-400` (WCAG floors, see [styling](styling.md)).
