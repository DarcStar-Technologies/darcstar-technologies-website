...
A soft diagonal highlight drifts across every frosted surface, like light glinting off glass. It's **one** `position: fixed` overlay (`.sheen-plane`, rendered in `+layout.svelte`) holding a single gradient band, `transform`-animated across the viewport, `clip-path`'d to the **union of the glass rects** — so every panel/nav/button shows the same beam at its true screen position (one coherent light source, not per-panel glints). The clip (rounded rects, radius read from each element's `border-radius`) is maintained in JS (`src/lib/glass-sheen.ts`) on scroll + resize + modal open; the plane sits at `z-70` above every layer so it catches the nav and modal, and `pointer-events: none` + the clip mean it only draws on glass. While the contact modal is open the clip switches to the modal's glass only, so page panels behind the scrim don't bleed sheen over it.

**Perf:** the beam is `transform`-only, so it's **compositor-only** — it never repaints the panels or re-runs their `backdrop-filter` blur (measured: steady-state Paint Δ ≈ 0; scrolling flat at baseline). The only cost is a small `getBoundingClientRect` pass per scroll frame (rAF-batched). Frozen under `prefers-reduced-motion`. Lesson: animate glass reflections as a **transform on a separate overlay**, never as a filter/`background-position` on the blurred element itself — the latter re-runs the blur every frame (see the [CosmicBackdrop note above](#dark-mode) for the same trap).
