...
// Query: {		"papers": *[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))] | order(select(darcstarAuthored == true => 0, 1) asc, publishedDate desc) [$offset...$end] {			_id,			title,			"slug": slug.current,			status,			darcstarAuthored,			"hasCommentary": coalesce(count(commentary) > 0, false),			venue,			publishedDate,			url,			doi,			arxivId,			codeUrl,			"abstract": select(count(string::split(abstract, " ")) > 50 => array::join(string::split(abstract, " ")[0...50], " ") + "…", abstract),			"authors": array::compact(authors[]->{ _id, name, "slug": slug.current }),			"topics": array::compact(topics[]->{ _id, title, "slug": slug.current })		},		"total": count(*[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))]),		"totalAll": count(*[_type == "paper" && defined(slug.current)]),		"topics": *[_type == "topic" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(title asc) {			"slug": slug.current,			title,			description		},		"teamAuthors": *[_type == "person" && kind != "external" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(coalesce(nameSortKey, lower(name)) asc) {			"value": slug.current,			"label": name,			"key": nameSortKey		},		"authorLabel": *[_type == "person" && defined(slug.current) && slug.current == $author][0].name	}
...
// Query: {		"papers": *[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))] | order(select(darcstarAuthored == true => 0, 1) asc, coalesce(publishedDate, "9999-12-31") asc) [$offset...$end] {			_id,			title,			"slug": slug.current,			status,			darcstarAuthored,			"hasCommentary": coalesce(count(commentary) > 0, false),			venue,			publishedDate,			url,			doi,			arxivId,			codeUrl,			"abstract": select(count(string::split(abstract, " ")) > 50 => array::join(string::split(abstract, " ")[0...50], " ") + "…", abstract),			"authors": array::compact(authors[]->{ _id, name, "slug": slug.current }),			"topics": array::compact(topics[]->{ _id, title, "slug": slug.current })		},		"total": count(*[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))]),		"totalAll": count(*[_type == "paper" && defined(slug.current)]),		"topics": *[_type == "topic" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(title asc) {			"slug": slug.current,			title,			description		},		"teamAuthors": *[_type == "person" && kind != "external" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(coalesce(nameSortKey, lower(name)) asc) {			"value": slug.current,			"label": name,			"key": nameSortKey		},		"authorLabel": *[_type == "person" && defined(slug.current) && slug.current == $author][0].name	}
...
// Query: {		"papers": *[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))] | order(coalesce(titleSortKey, lower(title)) asc) [$offset...$end] {			_id,			title,			"slug": slug.current,			status,			darcstarAuthored,			"hasCommentary": coalesce(count(commentary) > 0, false),			venue,			publishedDate,			url,			doi,			arxivId,			codeUrl,			"abstract": select(count(string::split(abstract, " ")) > 50 => array::join(string::split(abstract, " ")[0...50], " ") + "…", abstract),			"authors": array::compact(authors[]->{ _id, name, "slug": slug.current }),			"topics": array::compact(topics[]->{ _id, title, "slug": slug.current })		},		"total": count(*[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))]),		"totalAll": count(*[_type == "paper" && defined(slug.current)]),		"topics": *[_type == "topic" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(title asc) {			"slug": slug.current,			title,			description		},		"teamAuthors": *[_type == "person" && kind != "external" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(coalesce(nameSortKey, lower(name)) asc) {			"value": slug.current,			"label": name,			"key": nameSortKey		},		"authorLabel": *[_type == "person" && defined(slug.current) && slug.current == $author][0].name	}
...
// Query: *[_type == "paper" && slug.current == $slug][0] {		_id,		_updatedAt,		title,		"slug": slug.current,		status,		darcstarAuthored,		abstract,		commentary,		venue,		publishedDate,		url,		doi,		arxivId,		codeUrl,		"pdfUrl": pdf.asset->url,		"authors": array::compact(authors[]->{ _id, name, "slug": slug.current, role }),		"topics": array::compact(topics[]->{ _id, title, "slug": slug.current, description }),		"categories": array::compact(categories[]->{ _id, title, "slug": slug.current }),		seo	}
...
// Query: *[_type == "person" && kind != "external" && slug.current == $slug][0] {		_id,		_updatedAt,		name,		"slug": slug.current,		role,		image,		bio,		fullBio,		focusAreas,		responsibilities,		experience[]{ _key, title, organization, startYear, endYear, summary, url },		education[]{ _key, qualification, institution, year },		socialLinks[]{ label, url }	}
