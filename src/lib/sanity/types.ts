...
// Query: {		"papers": *[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))] | order(select(darcstarAuthored == true => 0, 1) asc, publishedDate desc) [$offset...$end] {			_id,			title,			"slug": slug.current,			status,			darcstarAuthored,			"hasCommentary": coalesce(count(commentary) > 0, false),			venue,			publishedDate,			url,			doi,			arxivId,			codeUrl,			"abstract": select(count(string::split(abstract, " ")) > 50 => array::join(string::split(abstract, " ")[0...50], " ") + "…", abstract),			"authors": array::compact(authors[]->{ _id, name, "slug": slug.current }),			"topics": array::compact(topics[]->{ _id, title, "slug": slug.current })		},		"total": count(*[_type == "paper" && defined(slug.current)		&& ($topic == null || $topic in topics[]->slug.current)		&& ($author == null || $author in authors[]->slug.current || authors[]->name match ($author + "*") || authors[]->nameSortKey match ($author + "*"))		&& ($origin == null			|| ($origin == "darcstar" && darcstarAuthored == true)			|| ($origin == "external" && darcstarAuthored != true))]),		"totalAll": count(*[_type == "paper" && defined(slug.current)]),		"topics": *[_type == "topic" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(title asc) {			"slug": slug.current,			title,			description		},		"teamAuthors": *[_type == "person" && kind != "external" && defined(slug.current) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0] | order(coalesce(nameSortKey, lower(name)) asc) {			"value": slug.current,			"label": name		},		"authorLabel": *[_type == "person" && defined(slug.current) && slug.current == $author][0].name	}
...
// Query: *[_type == "person" && defined(slug.current) && (name match ($q + "*") || nameSortKey match ($q + "*")) && count(*[_type == "paper" && defined(slug.current) && references(^._id)]) > 0]		| order(select(kind != "external" => 0, 1) asc, coalesce(nameSortKey, lower(name)) asc) [0...12] {			"value": slug.current,			"label": name		}
