<script lang="ts">
	...
	// and a shareable URL. Utility-page layout (a centred glass-panel), matching /contact. The
	// form is the shared LoginForm (the same one the navbar's LoginDialog renders), so the two
	// can't drift; sign-in is JS-driven (see LoginForm / auth-client.ts).
